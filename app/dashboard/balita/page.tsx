        ...
        <div className="space-y-4">
          {filteredData.length > 0 ? (
            filteredData.map((balita) => (
              <Card key={balita.id} className="flex flex-col border border-gray-100 shadow-sm rounded-xl bg-white overflow-hidden">
                ...
                <div className="px-5 py-4 flex items-center justify-between">
                  ...
                  <Link href={`/dashboard/balita/${balita.id}`} className="text-xs font-bold text-[#0d9488] hover:text-teal-900 transition-colors">
                    Lihat Detail &gt;
                  </Link>
                </div>
